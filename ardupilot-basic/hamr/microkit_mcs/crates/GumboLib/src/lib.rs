...
{
  valid_ipv4_udp(aframe) & udp_is_valid_direct_dst_port(aframe) &
    !(udp_is_mavlink(aframe))
}
...
{
  valid_arp(aframe) | valid_ipv4_udp_mavlink(aframe) |
...
verus! {
  ...
  {
    (valid_ipv4_udp_spec(aframe) && udp_is_valid_direct_dst_port_spec(aframe)) &&
      !(udp_is_mavlink_spec(aframe))
  }
  ...
  {
    (valid_arp_spec(aframe) || valid_ipv4_udp_mavlink_spec(aframe)) ||
