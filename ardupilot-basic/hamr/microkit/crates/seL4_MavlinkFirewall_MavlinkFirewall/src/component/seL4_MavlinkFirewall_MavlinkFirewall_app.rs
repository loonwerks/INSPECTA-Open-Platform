...
verus! {
  ...
  fn raw_eth_from_udp_frame(value: SW::UdpFrame_Impl) -> (r: SW::RawEthernetMessage)
        ensures
            mav_input_eq_output_spec(value, r),
  ...
  fn two_bytes_to_u16_exec(
    byte0: u8,
    byte1: u8) -> (r: u16)
    ensures
      r == two_bytes_to_u16_le_spec(byte0, byte1)
  {
    (((byte1) as u16) * 256u16 + ((byte0) as u16)) as u16
  }
  
  fn three_bytes_to_u32_exec(
    byte0: u8,
    byte1: u8,
    byte2: u8,
  ) -> (r: u32)
    ensures
      r == three_bytes_to_u32_spec(byte0, byte1, byte2)
  {
  ...
  fn can_send(udp_frame: SW::UdpFrame_Impl) -> (r: bool)
    ensures
      msg_is_blacklisted_spec(udp_frame.payload) == (r == false)
  {
    let msg = &udp_frame.payload;
    match msg[0] {
      // Mavlink v2
      0xFD => match three_bytes_to_u32_exec(msg[7], msg[8], msg[9]) {
          75 => two_bytes_to_u16_exec(msg[37], msg[38]) != 42650,
          76 => two_bytes_to_u16_exec(msg[38], msg[39]) != 42650,
          _ => true,
      },
      // Mavlink v1
      0xFE => match msg[5] {
          75 => two_bytes_to_u16_exec(msg[33], msg[34]) != 42650,
          76 => two_bytes_to_u16_exec(msg[34], msg[35]) != 42650,
          _ => true,
      }
      _ => true,
    }
  ...
  impl seL4_MavlinkFirewall_MavlinkFirewall {
    ...
    pub fn timeTriggered<API: seL4_MavlinkFirewall_MavlinkFirewall_Full_Api> (
      ...
      ensures
        ...
        api.In1.is_some() && GumboLib::msg_is_mav_cmd_flash_bootloader_spec(api.In1.unwrap().payload) ==>
          api.Out1.is_none(),
        // guarantee hlr_21_mav1_no_input
        !(api.In1.is_some()) ==> api.Out1.is_none(),
        // guarantee hlr_22_mav1_allow
        api.In1.is_some() && !(GumboLib::msg_is_blacklisted_spec(api.In1.unwrap().payload)) ==>
        ...
        api.In2.is_some() && GumboLib::msg_is_mav_cmd_flash_bootloader_spec(api.In2.unwrap().payload) ==>
          api.Out2.is_none(),
        // guarantee hlr_21_mav2_no_input
        !(api.In2.is_some()) ==> api.Out2.is_none(),
        // guarantee hlr_22_mav2_allow
        api.In2.is_some() && !(GumboLib::msg_is_blacklisted_spec(api.In2.unwrap().payload)) ==>
        ...
        api.In3.is_some() && GumboLib::msg_is_mav_cmd_flash_bootloader_spec(api.In3.unwrap().payload) ==>
          api.Out3.is_none(),
        // guarantee hlr_21_mav3_no_input
        !(api.In3.is_some()) ==> api.Out3.is_none(),
        // guarantee hlr_22_mav3_allow
        api.In3.is_some() && !(GumboLib::msg_is_blacklisted_spec(api.In3.unwrap().payload)) ==>
